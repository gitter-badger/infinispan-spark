...
class PartitionSuite extends FunSuite with Matchers {
   ...
   private def split(segmentsByServer: Map[SocketAddress, Set[Integer]], partitionPerServer: Integer) = {
      val props = new Properties()
      props.put(InfinispanRDD.PartitionsPerServer, partitionPerServer)
      splitter.split(segmentsByServer, props)
   }
   ...
   private def runTest(numSegments: Int, topology: Map[SocketAddress, Set[Integer]])(partitions: Int) {
      val numServers = topology.keySet.size
   ...
   private def reverse[K, V](m: Map[K, Set[V]]) = m.values.toSet.flatten.map(v => (v, m.keys.filter(m(_).contains(v)).toSet)).toMap
   ...
   private def createServerTopology(numSegments: Int, numOwners: Int, numServers: Int) = {
      ...
      val s: Map[Integer, Set[SocketAddress]] = (for (i <- 0 to numSegments - 1) yield int2Integer(i) -> serversStream.take(numOwners).toSet).toMap
      reverse(s)
   }
